...
@Component({
    selector: 'app-pages',
    templateUrl: './plan-pages.component.html',
    styleUrls: ['./plan-pages.component.scss'],
    providers: [SidenavMenuService]
})
export class PlanPagesComponent implements OnInit {
    ...
    public onSubmit(values: any): void {
        if (this.searchForm.valid) {
            if (this.category.id == 2  || this.category.id == 4) {
                this.router.navigate(['/search-videos', values.searchText, this.category.id]);
            } else {
                this.router.navigate(['/search', values.searchText, this.category.id]);
            }
        } else {
            this.snackBar.open('Add text to search', '×', { panelClass: 'error', verticalPosition: 'top', duration: 5000 });
        }
